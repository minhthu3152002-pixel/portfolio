...
> 🔎 **Tìm nhanh trong file:** để nhảy tới một dự án, dùng **Ctrl+F** (Cmd+F trên Mac)
> gõ **`PROJECT 3`** (hoặc tên dự án, ví dụ `Social Media`). Mỗi dự án — và cả khối
> `hero`, `about` — có một dòng mốc `"_section": "===== PROJECT 3 · SOCIAL MEDIA ====="`
> để mắt/Ctrl+F bắt được ngay. Dòng `"_toc"` ở **đầu file** liệt kê toàn bộ thứ tự.
> Các trường bắt đầu bằng dấu gạch dưới `_` (như `_toc`, `_section`) **chỉ là nhãn ghi chú**
> — không hiển thị trên web, có thể sửa chữ tuỳ ý, nhưng **đừng xoá dấu `"` hay dấu phẩy**.
