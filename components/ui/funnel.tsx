...
export function Funnel({ steps, lang }: { steps: FunnelStep[]; lang: Lang }) {
  return (
    <div className="liquid-glass rounded-[24px] p-6">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-1">
        {steps.map((step, i) => {
          const Icon = (step.icon && FUNNEL_ICONS[step.icon]) || Share2;
          const isLast = i === steps.length - 1;
          return (
            <div key={i} className="flex items-center gap-4 sm:flex-1 sm:flex-col sm:items-center sm:gap-0 sm:text-center">
              <span
              ...
              {!isLast && (
                <span aria-hidden className="shrink-0 text-muted/45 sm:mt-2.5 sm:self-start">
                  <ArrowDown size={16} strokeWidth={1.75} className="sm:hidden" />
                  <ArrowRight size={16} strokeWidth={1.75} className="mx-auto hidden sm:block" />
                </span>
              )}
            </div>
          );
